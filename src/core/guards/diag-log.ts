...
// 包化：app 名与版本由宿主 initDiagLog({ app, version }) 注入（原 import WEEBPAINT_VERSION）。
let _app = "app", _version = "0.0.0";
...
export function initDiagLog(opts: { app?: string; version?: string } = {}): void {
  if (opts.app) _app = opts.app; if (opts.version) _version = opts.version;
  ...
  note("boot", `${_version} ${g.location?.pathname ?? ""} online=${String(g.navigator?.onLine)}`);
