...
//   hasGallery() 从「device pref 开关」改读**真状态**：当前有活店（attachment attached，或 legacy
...
import type { GalleryAttachment } from "./gallery-attachment.ts";
// 包化：不再读宿主单例，改工厂——createGallery 把自己的 attachment 与 hasLiveStore 喂进来。
export interface GalleryCapability { galleryOnline(): boolean; hasGallery(): boolean; }
...
export function createGalleryCapability(deps: { attachment: Pick<GalleryAttachment, "state">; hasLiveStore: () => boolean; onLine?: () => boolean }): GalleryCapability {
  const onLine = deps.onLine ?? (() => (typeof navigator !== "undefined" ? navigator.onLine !== false : true));
  return {
    galleryOnline(): boolean {
      const s = deps.attachment.state();
      if (s.kind !== "attached" || !s.online) return false;
      return s.entry.kind === "folder" ? true : onLine();
    },
    /** 图库能力真状态：有活店 = true（folder 库不需要登录也是有库）；无库模式/absent = false。 */
    hasGallery(): boolean { return deps.hasLiveStore(); },
  };
}

// 换库/卸库（wp:gallery-changed）→ 能力事件重贴（消费方一处订阅不用改）。宿主 mount 时调一次。
export function wireCapabilityBroadcast(win: { addEventListener: Window["addEventListener"]; dispatchEvent: Window["dispatchEvent"] }): void {
  win.addEventListener("wp:gallery-changed", () => { try { win.dispatchEvent(new Event(GALLERY_CAPABILITY_EVENT)); } catch { /* noop */ } });
}
