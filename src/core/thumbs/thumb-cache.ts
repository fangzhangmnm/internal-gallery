...
export interface CachedThumb { token: string; blob: Blob; at: number; }
...
export interface ThumbCache {
  ...
  getOrFetch(name: string, token: string, source: ThumbSource): Promise<{ blob: Blob; fromCache: boolean }>;
...
export function createThumbCache(deps: ThumbCacheDeps): ThumbCache {
  ...
  async function read(name: string): Promise<CachedThumb | null> {
    try { const v = await deps.store.get(deps.keyOf(name)) as CachedThumb | undefined; return v && v.blob && v.token ? v : null; } catch { return null; }
  }
  async function write(name: string, token: string, blob: Blob): Promise<void> {
