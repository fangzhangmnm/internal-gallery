...
} from "../src/core/backup/library-backup.ts";
// zipPack/zipUnpack 是宿主 codec（vendored zip.js），不进包：下面用「JSON 假打包」验端口契约（真 zip 往返留在 WeebPaint）。
const zipPack = async (entries) => new Blob([JSON.stringify(await Promise.all(entries.map(async (e) => ({ path: e.path, data: Array.from(new Uint8Array(await e.data.arrayBuffer())) }))))]);
const zipUnpack = async (blob) => Object.fromEntries(JSON.parse(await blob.text()).map((e) => [e.path, new Uint8Array(e.data)]));
