...
import { copyTargetName, uniqueBareName as _uniqueBareName } from "../src/core/model/gallery-model.ts";
// 包化：裸名↔全名边界（WeebPaint sessionBareName/sessionFileName）成了注入参数；本用例按 WeebPaint 的边界跑，pin 住原语义。
const WP_NAMING = {
  bare: (n) => { const segs = (n || "Untitled").split("/").map((x) => x.replace(/[\\:*?"<>|]+/g, "_").trim()).filter(Boolean); if (!segs.length) segs.push("Untitled"); return segs.join("/"); },
  full: (b) => `${b}.ora`,
};
const uniqueBareName = (stem, occupied) => _uniqueBareName(stem, occupied, WP_NAMING);
